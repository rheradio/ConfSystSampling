---
title: 'RQ2: Scalability of the proposed goodness-of-fit test based on examining the SAT-solution distribution'
...
# Comparing the SAT-solution distribution test with the variable probability test
...
sample_size_comparison <- tibble(
  ...
  method = factor(c(rep("SAT-solution distribution", nrow(sat_dist_stats)), 
                        rep("Variable probabilities", nrow(var_prob_stats))),
                     levels = c("Variable probabilities", "SAT-solution distribution"))
)
...
ggplot(sample_size_comparison, aes(x=vars, y=sample_size)) +
  ...
  labs(
    ...
    subtitle = "The SAT-solution distribution test requires smaller\nsamples than the one based on variable probabilities",
    color = ""
  )  
